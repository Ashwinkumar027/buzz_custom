...
onMounted(() => {
  setTimeout(() => {
    const input = document.querySelector("#phone-0");
...
const addAttendee = () => {
    if (attendees.value.length >= 1) {
        return; // stop if one attendee already exists
    }
	...
	() => {
		if (attendees.value.length === 0 && props.availableTicketTypes.length > 0) {
			...
			if (userResource.data) {
				...
				newAttendee.mobile = userResource.data.mobile_no || "";
			}
...
function prefillAttendee(field) {
	...
	if (field === "mobile" && !first.mobile) first.mobile = guestPhone.value;
}
...
async function submit() {
	...
	const validationErrors = validateForm();
		...
		if (itiInstance) {
    		...
    		toast.error(__("Enter valid phone number"));
    	return;
  		}

  		const fullPhone = itiInstance.getNumber(); // +9194945929544

  		const nationalNumber = window.intlTelInputUtils
    		? itiInstance.getNumber(window.intlTelInputUtils.numberFormat.NATIONAL)
  		...
  		if (itiInstance) {
  			...
  			const fullPhone = itiInstance.getNumber(); // 🔥 ALWAYS USE THIS

  			attendees.value[0].mobile = fullPhone;
  			guestPhone.value = fullPhone;
	...
	const attendees_payload = attendees.value.map((attendee) => {
		...
		cleanAttendee.mobile = attendee.mobile;

		return {
        	full_name: cleanAttendee.full_name,
  			email: cleanAttendee.email,
  			mobile: cleanAttendee.mobile,
  			ticket_type: cleanAttendee.ticket_type,
  			add_ons: cleanAttendee.add_ons,
  			custom_fields: cleanAttendee.custom_fields
};
	});
	...
	if (props.isGuestMode) {
		...
		// ✅ Guest phone check (already validated by intl-tel-input)
		if (isPhoneOtp.value && !guestPhone.value) {
  			toast.error(__("Enter valid phone number"));
  			return;
		}

		// ✅ Attendee phone check
		for (const attendee of attendees.value) {
  			if (!attendee.mobile || !attendee.mobile.trim()) {
    			toast.error(__("Attendee mobile number is required"));
    			return;
  			}
		}
