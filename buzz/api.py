...
def process_booking(
	...
	if not mobile.startswith("+"):
		frappe.throw("Phone must include country code (e.g. +9198...)")

	#  global validation (8-15 digits)
	if not re.match(r"^\+\d{8,15}$", mobile):
	...
	if len(attendees) != 1:
		frappe.throw("Only one attendee allowed per booking")

	mobiles = [a.get("mobile") for a in attendees if a.get("mobile")]

	if len(mobiles) !=len(set(mobiles)):
		frappe.throw("only one ticket is allowed per phone number")				
	for attendee in attendees:
		mobile = attendee.get("mobile")
		#prevent previous event attendees
		mobile = mobile.strip()
		...
		if frappe.db.exists(
			...
			frappe.throw("This phone number has already booked an event.")
			
		add_ons_data = attendee.get("add_ons", None)
		add_ons = None
